...
options = trainingOptions('adam',...        % 指定使用的优化器为 Adam（自适应矩估计
    ...
    MaxEpochs = 80,...                      % 指定训练的最大周期数（epochs）
    MiniBatchSize = 4,...                   % 每次训练时使用的小批量（mini-batch）大小
    GradientDecayFactor = 0.8,...           % Adam 优化算法中的一个参数，控制梯度的衰减因子。
    SquaredGradientDecayFactor = 0.99,...  % Adam 优化器中的一个参数，控制平方梯度的衰减因子。
    LearnRateSchedule = "piecewise",...     % 设置学习率调度方式。"piecewise" 意味着学习率会按预设的计划在某些周期后下降。
    InitialLearnRate = 0.0001,...           % 初始学习率用于控制每次更新模型参数时的步长
    LearnRateDropPeriod = 10,...            % 该参数表示学习率下降的周期数。
    LearnRateDropFactor = 0.7,...           % 学习率下降因子。
    ExecutionEnvironment= executionEnvironment, ... 
    PreprocessingEnvironment = 'parallel',...      % 指定是否启用数据并行预处理。如果设置为 'parallel'，则在数据加载时启用并行处理，以加速数据预处理。
