...
The operation \(\otimes\) represents the element-wise multiplication between state vectors, and \(c_{j}\) is used as a normalization factor to dynamically adjust the relative weights of different targets during the fusion process.
This mathematical representation captures the essential mechanism for maintaining consistent multi-target tracking through temporal state propagation and weighted information fusion.
...
Where \(c_{j}(m)\) is the class probability vector for detection \(m\), \(\otimes\) represents element-wise multiplication, \(\mu(k-1)\) is the class probability vector for the \(k-1\)th step, and \(\mu(k)\) is the class probability vector for the \(k\)-th step.
