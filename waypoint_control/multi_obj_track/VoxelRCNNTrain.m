...
lds = fileDatastore(lidarDataPath,"ReadFcn",@(x) pcread(x));

% 加载边界框标签
gtPath = fullfile(labelDataPath,'CarlaSetLidarGroundTruth.mat');
bboxLabels = load(gtPath,'LabelData');
bboxTable = bboxLabels.LabelData;
bboxTable = bboxTable(:,2:4);
% 显示全视图点云
figure
ptCld = preview(lds);
