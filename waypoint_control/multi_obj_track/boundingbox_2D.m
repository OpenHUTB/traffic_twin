...
for fileIdx = 1:length(matFiles)
    ...
    for i = 1:numel(cameraData)
        ...
        % 将扩展名从 jpg 修改为 png
        [directory, name, ~] = fileparts(imgPath);
        imgPathPng = strcat(name, '.png'); % 修改扩展名为 png
        fullImg = fullfile(directory, imgPathPng);
        fullImgPath = fullfile(dataPath, fullImg); % 构造完整路径
